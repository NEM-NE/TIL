...
레지스터 => 계산 전용 메모장인 `어큐뮬레이터`(다음 연산을 위해 이전 연산 결과를 저장할 때 사용), 계산 외에도 여러가지 일을 하는 범용 레지스터가 존재(이외에도 더 있음)
...
쉬프트 => 모든 비트를 오른쪽 or 왼쪽으로 n칸 씩 전체 이동한다.

부호 비트 => 최상위 비트에 0(양수), 1(음수)를 넣어 음양 파악
...
산술 쉬프트는 부호비트를 고려해야하기 때문에 쉬프트 하기 까다로움

![](https://t1.daumcdn.net/cfile/tistory/115D4B194C3B03E9B0)

위 방식처럼 진행

그러나 왼쪽 쉬프트 같은 경우 자릿수를 초과하여 부호비트를 못넣게 되면 `오버플로우`가 발생한다.

오버 플로우가 발생하면 `스테이터스 레지스터` 안의 `오버플로 플래그`가 설정된다.

마지막으로 `서큘러 시프트`는 양쪽의 끝이 서로 연결되어 있는 구조이다.

![](https://4.bp.blogspot.com/-UEJEcgdGO1Q/W9xxPyNVAFI/AAAAAAAAAPQ/1JnsLtj_bpsiZSnCpEEOSPJBkxbajwAHACLcBGAs/s1600/Image%2B%255B11%255D.png)

데이터 전송 명령

CPU와 메모리 사이에 행해지는 읽기/쓰기 작업 or 레지스터끼리 데이터 이동 명령

입출력 명령

I/O 포트를 통해 외부장치와 데이터를 주고받기 위한 명령
